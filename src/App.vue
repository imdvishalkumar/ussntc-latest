...
export default {
  ...
  setup() {
    ...
    const value = sessionStorage.getItem("verifiedCode");

    if (value) {
      const parsedValue = JSON.parse(value);
      // Assuming you have a cartStore already available
      cartStore.marketing.campaign_access_code = parsedValue.code;
    }
      ...
      if (validateForm()) {
        try {
          ...
          if (response.status == "success") {
            ...
            console.log("Code verified successfully:", response.code);
            cartStore.marketing.campaign_access_code = response.code;
          } else {
    ...
    function setFontStyle(fontStyleUrl, fontStyle) {
      ...
      const existingLink = document.querySelector(
        'link[rel="stylesheet"][data-dynamic-font]'
      );
      if (existingLink) {
        existingLink.remove();
      }
