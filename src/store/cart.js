...
export const useCartStore = defineStore("cart", {
  ...
  getters: {
    ...
    subTotal: (state) => {
      return state.cartItems.reduce((total, item) => {
        ...
        // Handle donation price (if `item_id === Donation`, use price)
        const donationTotal = item.donationDescription === "Donation" ? parseFloat(item.price || 0) : 0;
  ...
  actions: {
    ...
    updateMarketingData({ campaign_access_code, emailAssociated, donatingOption, partner_code , how_you_heard, buying_as,affiliate_code }) {
      if (campaign_access_code) this.marketing.campaign_access_code = campaign_access_code;
